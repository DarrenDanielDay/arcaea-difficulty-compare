...
export const CompareTask = <T extends unknown>({ store, taskInit, renderEntity }: CompareTaskProp<T>) => {
  ...
  return (
    <div>
      ...
      <div class="my-3 small">
        由于常见定数范围内的谱面数量会导致需要的比较次数通常较多，这里提供了<mark>进度保存</mark>
        功能。如果您对当前已经选择的结果确认无误，您可以点击<mark>保存进度</mark>
        按钮来保存当前的进度到存档中。如果您不小心选错了，<strong>暂不支持撤销操作</strong>，您只能从某个存档恢复进度。
      </div>
      <Balancer<T>
        left={left.data}
        right={right.data}
